...
def toChar(i): return chr(i)

def decrypt(k1,k2,k3):
    global S,n
    K = [k1,k2,k3]
    D = [0]*n
...
for k1 in LOWERCASE:
    for k2 in LOWERCASE:
        for k3 in LOWERCASE:
            D = decrypt(k1,k2,k3)
            #DECRYPTS.append(D)
            if len(intersect(D.split(),WORDS)) > 5:
                print(chr(k1),chr(k2),chr(k3)," - ",sumASCII(D),sumDecr(k1,k2,k3), " - ",D)
